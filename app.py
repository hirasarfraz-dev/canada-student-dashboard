...
def load_data(year: int) -> pd.DataFrame:
    ...
    for key, code in INDICATORS.items():
        ...
        if not df.empty:
            ...
            frames.append(df[["iso3", "country", "region", key]])
...
def main():
    ...
    tab_map, tab_rank, tab_explore, tab_data = st.tabs(
        ["🗺️ World Map", "📊 Rankings", "🔍 Explorer", "📋 Data"]
    )
    ...
    with tab_map:
        ...
        fig_map = px.choropleth(
            ...
            hover_data={"iso3": False, "composite_score": ":.1f", "rank": True,
                        "gdp_per_capita": ":,.0f", "tertiary_enrollment": ":.1f", "youth_unemployment": ":.1f"},
            color_continuous_scale="RdYlGn",
            range_color=[0, 100],
            labels={"composite_score": "Score", "gdp_per_capita": "GDP per Capita (USD)",
                    "tertiary_enrollment": "Tertiary Enroll. (%)", "youth_unemployment": "Youth Unemp. (%)"},
        )
    ...
    with tab_explore:
        ...
        exp_cols = list(INDICATOR_LABELS.keys())
        ...
        if not row.empty:
            ...
            raw_vals = [
                r.get("s_youth",    0) or 0,
                r.get("s_gdp",      0) or 0,
                r.get("s_tertiary", 0) or 0,
                r.get("s_unemploy", 0) or 0,
                r.get("s_internet", 0) or 0,
            ]
    ...
    with tab_data:
        ...
        display = scored[[
            ...
            "total_population": "Total Population", "tertiary_enrollment": "Tertiary Enrollment (%)",
            "youth_unemployment": "Youth Unemployment (%)", "internet_users": "Internet Users (%)",
        }).dropna(subset=["Score (0–100)"]).sort_values("Rank")

        st.dataframe(
            display.style.format({
                "Score (0–100)": "{:.1f}", "GDP per Capita (USD)": "${:,.0f}",
                "Total Population": "{:,.0f}", "Tertiary Enrollment (%)": "{:.1f}",
                "Youth Unemployment (%)": "{:.1f}", "Internet Users (%)": "{:.1f}",
        ...
        st.download_button(
            ...
            file_name=f"canada_student_sources_{year}.csv", mime="text/csv",
        )
